...
위 법률은 2023년 9월 15일에 폐지되었으나, 불필요한 개인정보 보관으로 인한 위험을 줄이고 싶은 사이트라면
이 모듈을 사용해서 휴면회원을 주기적으로 정리해 줄 수 있습니다.
...
- Rhymix 2.0 이상, PHP 7.2 이상
- 구 XE 환경에서 사용시 발생하는 오류에 대해서는 질문 받지 않습니다. Rhymix로 업그레이드 후 사용해 주세요.

안내메일을 대량으로 발송하는 경우에는 코어의 "알림 설정"에서 대량메일 전문 API를 연동하는 것을 권장합니다.

버그를 신고하실 때는 사용중이신 Rhymix 버전, PHP 버전, 호스팅 정보 (웹호스팅 업체 또는 단독서버/클라우드서버),
서버의 에러로그, 브라우저의 개발자도구로 AJAX 요청을 캡쳐했을 때 나오는 내용 등
가능한 많은 정보를 포함하여 이슈를 작성해 주시기 바랍니다.
